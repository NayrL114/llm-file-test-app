...
**Review Timestamp:** 2026-01-29 20:24:09

## High-Level Summary

This repo is a React + Vite frontend with a Node/Express backend in a simple monorepo (npm workspaces). The app exposes two primary user flows: a multi-model Chat Console and a Resume Extractor that performs schema-based extraction from uploaded files. The backend proxies requests to OpenAI, Gemini, and Claude, and persists all requests in a SQLite history database.
...
- **Schema extraction:** Command specs in `backend/commands/*.json` define prompts and JSON schema for structured extraction.
- **API routes:**
...
- **Resume Extract:** Uploads files for extraction, displays editable structured output, and lists file history.
