...
function normalizeResume(resume) {
  // Ensure all required keys exist, even if backend/model changes
  const r = resume || {};
...
export default function ResumeExtractPage() {
  ...
  // editable draft state (mirrors extractedResult but user-editable)
  const [draftResumes, setDraftResumes] = useState([]); // array of resume objects
  const [selectedResumeIndex, setSelectedResumeIndex] = useState(0);

  // collapsible state for optional sections
  const [collapsedOptional, setCollapsedOptional] = useState({}); // key: `${resumeIdx}:${sectionIdx}` => boolean

  const apiBase = useMemo(() => import.meta.env.VITE_API_BASE_URL || "", []);

  const currentDraftResume = useMemo(() => {
    if (!draftResumes.length) return null;
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      // Use your new workflow-separated command file
      ...
      // initialize optional collapse state: default expanded (false)
      const initCollapsed = {};
      normalized.forEach((r, rIdx) => {
        (r.optional_sections || []).forEach((_, sIdx) => {
          initCollapsed[`${rIdx}:${sIdx}`] = false;
        });
      });
      setCollapsedOptional(initCollapsed);
    } catch (err) {
  ...
  return (
    <div>
      ...
      <p>
        Upload a resume, extract structured JSON (schema: <code>resume_extraction_v1</code>), then edit it by section.
      </p>
      ...
      <section style={{ border: "1px solid #ddd", borderRadius: 8, padding: 16, marginBottom: 16 }}>
        ...
        {!!draftResumes.length && (
          <div style={{ marginTop: 12, color: "#444" }}>
            Extracted {draftResumes.length} resume{draftResumes.length > 1 ? "s" : ""} from this file.
          </div>
        )}
      </section>
      ...
      {/* Sectioned editor */}
      <section style={{ border: "1px solid #ddd", borderRadius: 8, padding: 16, marginBottom: 16 }}>
        <h2 style={{ marginTop: 0 }}>2) Edit by Section</h2>

        {!currentDraftResume ? (
          <p style={{ margin: 0 }}>No extracted data yet.</p>
        ) : (
          <>
            {/* Personal Profile */}
            ...
            {currentDraftResume.optional_sections.length > 0 && (
              <div style={{ border: "1px solid #eee", borderRadius: 8, padding: 12 }}>
                <h3 style={{ marginTop: 0 }}>Optional Sections</h3>

                {currentDraftResume.optional_sections.map((sec, secIdx) => {
                  const key = `${selectedResumeIndex}:${secIdx}`;
                  const isCollapsed = !!collapsedOptional[key];

                  return (
                    <div key={key} style={{ borderTop: "1px solid #eee", paddingTop: 12, marginTop: 12 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                        <strong>{sec.section_name || `Section #${secIdx + 1}`}</strong>
                        <button
                          type="button"
                          onClick={() =>
                            setCollapsedOptional((prev) => ({ ...prev, [key]: !prev[key] }))
                          }
                        >
                      ...
                      {!isCollapsed && (
                        <div style={{ marginTop: 8 }}>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setResumeAtIndex(selectedResumeIndex, (r) => {
                                r.optional_sections[secIdx].content = e.target.value;
                                return r;
                              })
                            }
                          />
      ...
      <section style={{ border: "1px solid #ddd", borderRadius: 8, padding: 16 }}>
        <h2 style={{ marginTop: 0 }}>3) JSON Preview</h2>

        {!draftResumes.length ? (
          <p style={{ margin: 0 }}>No extracted JSON yet.</p>
        ) : (
          <>
